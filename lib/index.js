...
void function () {
  var CoffeeScript, esprima, estraverse, fs, path, PRELUDE, relativeResolve, resolve, resolvePath, wrap;
  path = require('path');
  fs = require('fs');
  ...
  exports.cjsify = function (entryPoint, root, options) {
    var ast, cache$, canonicalName, exportExpression, ext, extensions, extname, fileContents, filename, handler, handlers, lhs, outputProgram, processed, source, worklist;
    if (null == root)
      root = process.cwd();
    if (null == options)
      options = {};
    ...
    if (null != options['export']) {
      ...
      lhs = exportExpression.type === 'Identifier' ? {
        type: 'MemberExpression'({
          computed: false,
          object: {
            type: 'Identifier',
