...
void function () {
  var badRequireError, bundle, CoffeeScript, CORE_DIR, esprima, estraverse, fs, isCore, path, PRELUDE, relativeResolve, relativeResolveSync, resolve, resolvePath, resolvePathSync, wrapFile;
  path = require('path');
  fs = require('fs');
  ...
  exports.cjsify = function (entryPoint, root, options, cb) {
    if (null == root)
      root = process.cwd();
    if (null == options)
      options = {};
    ...
    return process.nextTick(function () {
      var e;
      try {
        return cb(null, exports.cjsifySync(entryPoint, root, options));
      } catch (e$) {
        e = e$;
        return cb(e);
      }
  ...
  exports.cjsifySync = function (entryPoint, root, options) {
    ...
    return bundle(processed, relativeResolveSync(extensions, root, entryPoint), options);
  };
