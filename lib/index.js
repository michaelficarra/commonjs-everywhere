...
void function () {
  var badRequireError, bundle, canonicalise, cjsify, cjsifySync, CoffeeScript, CORE_DIR, esprima, estraverse, fs, isCore, path, PRELUDE, relativeResolve, relativeResolveSync, resolve, resolvePath, resolvePathSync, traverseDependencies, traverseDependenciesSync, wrapFile;
  path = require('path');
  fs = require('fs');
  ...
  traverseDependencies = function (entryPoint, root, options, cb) {
    ...
    return process.nextTick(function () {
      var e;
      try {
        return cb(null, traverseDependenciesSync(entryPoint, root, options));
      } catch (e$) {
        e = e$;
        return cb(e);
      }
  ...
  traverseDependenciesSync = function (entryPoint, root, options) {
    var aliases, ast, cache$, cache$1, canonicalName, ext, extensions, extname, fileContents, filename, handler, handlers, processed, source, worklist;
    if (null == root)
      root = process.cwd();
    if (null == options)
      options = {};
    ...
    worklist = [[
        ...
        canonicalise(root, entryPoint)
      ]];
    processed = {};
    while (worklist.length) {
      cache$1 = worklist.pop();
      filename = cache$1[0];
      canonicalName = cache$1[1];
