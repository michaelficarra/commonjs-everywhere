...
void function () {
  var CoffeeScript, CORE_DIR, esprima, estraverse, fs, isCore, path, PRELUDE, relativeResolve, resolve, resolvePath, wrap;
  path = require('path');
  fs = require('fs');
  ...
  exports.cjsify = function (entryPoint, root, options) {
    var ast, cache$, canonicalName, exportExpression, ext, extensions, extname, fileContents, filename, handler, handlers, lhsExpression, outputProgram, processed, requireEntryPoint, source, worklist;
    if (null == root)
      root = process.cwd();
    if (null == options)
      options = {};
