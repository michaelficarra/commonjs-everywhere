...
void function () {
  var $0, build, cache$, CJSEverywhere, escodegen, escodegenCompactFormat, escodegenDefaultFormat, fs, Jedediah, optionParser, options, originalEntryPoint, path, positionalArgs, root, startBuild, stdinput;
  fs = require('fs');
  path = require('path');
  ...
  build = function (entryPoint, processed) {
    var bundled, cache$1, code, esmangle, file, map, newDeps;
    if (null == processed)
      processed = {};
    ...
    cache$1 = escodegen.generate(bundled, {
      ...
      sourceMapRoot: null != options.sourceMapFile ? path.relative(path.dirname(options.sourceMapFile), root) || '.' : void 0,
