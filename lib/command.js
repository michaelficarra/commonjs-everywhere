...
void function () {
  var $0, _, aliasPair, buildBundle, bundle, cache$3, cache$4, dep, deps, escodegenFormat, fs, handlerPair, knownOpts, match, nopt, opt, optAliases, options, originalEntryPoint, path, positionalArgs, startBuild, stdinput, traverseDependencies;
  fs = require('fs');
  path = require('path');
  nopt = require('nopt');
  ...
  buildBundle = function () {
    var cache$5, code, datauri, map, sourceMappingUrl;
    traverseDependencies(originalEntryPoint, options);
    cache$5 = bundle(originalEntryPoint, options);
    code = cache$5.code;
    map = cache$5.map;
    ...
    if (options.output) {
      return fs.writeFileSync(options.output, code);
    } else {
      return process.stdout.write('' + code + '\n');
    }
  };
  startBuild = function () {
    var building, cache, cache$5, moduleUids, processed, uids, watching;
    process.on('exit', function () {
      ...
      return fs.writeFileSync(options.cachePath, JSON.stringify(cache));
    });
    process.on('uncaughtException', function (e) {
      try {
        fs.unlinkSync(options.cachePath);
      } catch (e$) {
      }
      options.processed = {};
      throw e;
    ...
    if (fs.existsSync(options.cachePath)) {
      ...
      cache$5 = cache;
      processed = cache$5.processed;
      uids = cache$5.uids;
      moduleUids = cache$5.moduleUids;
      cache$5;
    }
    if (!processed || moduleUids !== options.moduleUids) {
      processed = {};
      uids = {
        next: 1,
        names: {}
    ...
    options.uids = uids;
    options.uidFor = function (name) {
      var uid;
      if (!options.moduleUids)
        return name;
      if (!{}.hasOwnProperty.call(uids.names, name)) {
        uid = uids.next++;
        uids.names[name] = uid;
      }
