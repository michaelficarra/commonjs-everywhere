...
void function () {
  var $0, _, btoa, build, cache$, CJSEverywhere, dep, deps, escodegen, escodegenCompactFormat, escodegenDefaultFormat, fs, Jedediah, optionParser, options, originalEntryPoint, path, positionalArgs, root, startBuild, stdinput;
  fs = require('fs');
  path = require('path');
  ...
  build = function (entryPoint, processed) {
    var bundled, cache$1, cache$2, canonicalName, code, datauri, e, esmangle, file, fileContents, filename, map, newDeps, sourceMappingUrl;
    if (null == processed)
      processed = {};
    try {
    ...
    cache$1 = escodegen.generate(bundled, {
    ...
    if ((options.sourceMap || options.inlineSourceMap) && options.inlineSources)
      for (filename in processed) {
        if (!isOwn$(processed, filename))
          continue;
        {
          cache$2 = processed[filename];
          canonicalName = cache$2.canonicalName;
          fileContents = cache$2.fileContents;
        }
