...
void function () {
  var $0, _, aliasPair, btoa, build, cache$3, cache$4, CJSEverywhere, dep, deps, escodegen, escodegenDefaultFormat, fs, handlerPair, knownOpts, match, nopt, opt, optAliases, options, originalEntryPoint, path, positionalArgs, root, startBuild, stdinput;
  fs = require('fs');
  path = require('path');
  ...
  build = function (entryPoint, processed) {
    ...
    cache$5 = escodegen.generate(bundled, {
      ...
      format: options.minify ? escodegen.FORMAT_MINIFY : escodegenDefaultFormat
    });
    code = cache$5.code;
    map = cache$5.map;
